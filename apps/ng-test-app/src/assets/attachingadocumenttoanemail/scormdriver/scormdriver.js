/* Copyright © 2003-2016 Rustici Software, LLC  All Rights Reserved. www.scorm.com */
var VERSION = "7.0.3";
    ...
    TinCan.Utils = {
        ...
        convertISO8601DurationToMilliseconds: function(ISO8601Duration) {
            var isValueNegative = (ISO8601Duration.indexOf("-") >= 0),
                ...
                hours, minutes, seconds, durationInMilliseconds;
        ...
        convertMillisecondsToISO8601Duration: function(inputMilliseconds) {
            var hours, minutes, seconds, i_inputMilliseconds = parseInt(inputMilliseconds, 10),
                i_inputCentiseconds, inputIsNegative = "",
                rtnStr = "";
        ...
        parseURL: function(url, cfg) {
            var isRelative = url.charAt(0) === "/",
                _reURLInformation = ["(/[^?#]*)", "(\\?[^#]*|)", "(#.*|)$"],
                reURLInformation, match, result, paramMatch, pl = /\+/g,
                search = /([^&=]+)=?([^&]*)/g,
                decode = function(s) {
...
(function() {
    ...
    LRS.prototype = {
        ...
        retrieveState: function(key, cfg) {
            ...
            var requestParams = {},
                requestCfg = {},
                requestResult, callbackWrapper, requestHeaders, self = this;
        ...
        retrieveStateIds: function(cfg) {
            ...
            var requestParams = {},
                requestCfg, requestHeaders, requestResult, callbackWrapper;
            cfg = cfg || {};
            requestHeaders = cfg.requestHeaders || {};
        ...
        retrieveActivity: function(activityId, cfg) {
            ...
            var requestCfg = {},
                requestResult, callbackWrapper, requestHeaders;
            requestHeaders = cfg.requestHeaders || {};
            requestCfg = {
                url: "activities",
                method: "GET",
...
(function() {
    ...
    Context.prototype = {
        ...
        init: function(cfg) {
            this.log("init");
            var i, directProps = ["registration", "revision", "platform", "language", "extensions"],
...
(function() {
    ...
    var THIS_LIBRARY = {
        ...
        nativeRequest, xdrRequest, requestComplete, __delay, env = {},
        ...
        verbDisplay = {},
        launchParameters = ["endpoint", "fetch", "actor", "activityId", "registration"],
        isInteger;
        ...
        postFetch: function(callback) {
            ...
            if (callback) {
                cbWrapper = function(err, xhr) {
                    ...
                    var parsed, responseContent = xhr.responseText,
                        responseContentType;
                    if (err !== null) {
                        if (err === 0) {
            ...
            return this._fetchRequest(this._fetch, {
                method: "POST"
            }, cbWrapper);
        },
        ...
        saveLearnerPrefs: function(callback) {
            ...
            if (callback) {
                cbWrapper = function(err, result) {
                    ...
                    self._learnerPrefs.etag = TinCan.Utils.getSHA1String((typeof self._learnerPrefs.contents === "object" && TinCan.Utils.isApplicationJSON(self._learnerPrefs.contentType)) ? JSON.stringify(self._learnerPrefs.contents) : self._learnerPrefs.contents);
                    callback(null, result);
                };
            }
            result = this._lrs.saveAgentProfile(AGENT_PROFILE_LEARNER_PREFS, this._learnerPrefs.contents, {
                agent: this._actor,
...
function TCAPI_RecordInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime, TCAPIInteractionType, strAlternateResponse, strAlternateCorrectResponse) {
    var blnTempResult, intInteractionIndex, strResult, actObj = {},
        stmt, activityName, interactionActivityId = strID,
...
function CMI5_SaveState(key, value, contentType, includeRegistration) {
    ...
    var saveStateCfg = {
        ...
        result, errCode, errMesg, errDiag;
...
function CMI5_CommitData() {
    ...
    var i, st, auActivityId, errMesg, errCode, errDiag, saveCommittedStatus = false,
...
function CMI5_SetBookmark(location, description) {
    ...
    var st, targetId = CreateValidIdentifier(location);
    if (targetId !== "") {
...
function CMI5_GetPreviouslyAccumulatedTime() {
    ...
    if (CMI5_TOTAL_PREV_DURATION === null) {
        CMI5_TOTAL_PREV_DURATION = CMI5_RetrieveState(CMI5_STATE_TOTAL_TIME);
        WriteToDebug("   fetched previous duration for caching: " + CMI5_TOTAL_PREV_DURATION);
        if (CMI5_TOTAL_PREV_DURATION === "") {
...
function CMI5_RecordInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime, interactionType, strAlternateResponse, strAlternateCorrectResponse) {
    var st;
    CMI5_ClearErrorInfo();
    if (cmi5.getLaunchMode() !== "Normal") {
    ...
    if (strLearningObjectiveID !== "") {
        st.context.contextActivities.grouping = st.context.contextActivities.grouping || [];
        st.context.contextActivities.grouping.push(new TinCan.Activity({
...
function CMI5_RecordTrueFalseInteraction(strID, blnResponse, blnCorrect, blnCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordTrueFalseInteraction strID=" + strID + ", strResponse=" + strResponse + ", blnCorrect=" + blnCorrect + ", strCorrectResponse=" + strCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    var strResponse = "",
        strCorrectResponse = null;
    if (blnResponse === true) {
        strResponse = "true";
...
function CMI5_RecordMultipleChoiceInteraction(strID, aryResponse, blnCorrect, aryCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordMultipleChoiceInteraction strID=" + strID + ", aryResponse=" + aryResponse + ", blnCorrect=" + blnCorrect + ", aryCorrectResponse=" + aryCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    var strResponse = "",
        strCorrectResponse = "",
        i;
...
function CMI5_RecordFillInInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordFillInInteraction strID=" + strID + ", strResponse=" + strResponse + ", blnCorrect=" + blnCorrect + ", strCorrectResponse=" + strCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
...
function CMI5_RecordMatchingInteraction(strID, aryResponse, blnCorrect, aryCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordMatchingInteraction strID=" + strID + ", aryResponse=" + aryResponse + ", blnCorrect=" + blnCorrect + ", aryCorrectResponse=" + aryCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    var strResponse = "",
        strCorrectResponse = "",
        i;
    for (i = 0; i < aryResponse.length; i += 1) {
...
function CMI5_RecordPerformanceInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordPerformanceInteraction strID=" + strID + ", strResponse=" + strResponse + ", blnCorrect=" + blnCorrect + ", strCorrectResponse=" + strCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    strResponse = new String(strResponse);
    if (strCorrectResponse == null) {
        strCorrectResponse = "";
    }
...
function CMI5_RecordSequencingInteraction(strID, aryResponse, blnCorrect, aryCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordSequencingInteraction strID=" + strID + ", aryResponse=" + aryResponse + ", blnCorrect=" + blnCorrect + ", aryCorrectResponse=" + aryCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    var strResponse = "",
        strCorrectResponse = "",
        i;
    for (i = 0; i < aryResponse.length; i += 1) {
...
function CMI5_RecordLikertInteraction(strID, response, blnCorrect, correctResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordLikertInteraction strID=" + strID + ", response=" + response + ", blnCorrect=" + blnCorrect + ", correctResponse=" + correctResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
    var strResponse = (response.Long !== "" ? response.Long : response.Short),
        strCorrectResponse = "";
    if (correctResponse !== null) {
...
function CMI5_RecordNumericInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime) {
    WriteToDebug("In CMI5_RecordNumericInteraction strID=" + strID + ", strResponse=" + strResponse + ", blnCorrect=" + blnCorrect + ", strCorrectResponse=" + strCorrectResponse + ", strDescription=" + strDescription + ", intWeighting=" + intWeighting + ", intLatency=" + intLatency + ", strLearningObjectiveID=" + strLearningObjectiveID + ", dtmTime=" + dtmTime);
...
function CMI5_CreateInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime, interactionType, strAlternateResponse, strAlternateCorrectResponse) {
...
function CMI5_ResetStatus() {
    ...
    for (i = 0; i < CMI5_STATEMENT_QUEUE.length; i += 1) {
        tempSt = CMI5_STATEMENT_QUEUE[i];
        if (tempSt.target.id = auActivityId && (tempSt.verb.id === CMI5_VERB_ID_COMPLETED || tempSt.verb.id === CMI5_VERB_ID_PASSED || tempSt.verb.id === CMI5_VERB_ID_FAILED)) {
            CMI5_STATEMENT_QUEUE.splice(i, i + 1);
        }
    }
    return true;
...
function ShowDebugWindow() {
    ...
    if (winDebug === null) {
        ...
        if (winDebug.addEventListener || winDebug.attachEvent) {
            winDebug[winDebug.addEventListener ? 'addEventListener' : 'attachEvent']((winDebug.attachEvent ? 'on' : '') + 'load', renderLog, false);
        }
        renderLog();
