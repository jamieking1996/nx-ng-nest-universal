...
function SCORM2004_CreateValidIdentifier(str){return CreateUriIdentifier(str);}
...
if(origResult.err===null){return origResult;}
if(typeof origResult.err!=="undefined"&&origResult.err===0){for(prop in cfg.params){if(cfg.params.hasOwnProperty(prop)){pairs.push(prop+"="+encodeURIComponent(cfg.params[prop]));}}
...
stmt={verb:TCAPI_VERB_ANSWERED,object:actObj,context:{contextActivities:{parent:tincan.activity,grouping:{id:tincan.activity.id+'-'+strLearningObjectiveID}}}};if(strResponse!==null){stmt.result={response:strResponse,success:blnCorrect};}
...
CMI5_PENDING_STATUS.success=false;st=cmi5.failedStatement();CMI5_SetDurationInStatement(st);CMI5_STATEMENT_QUEUE.push(st);return true;}
...
CMI5_PENDING_STATUS.success=true;st=cmi5.passedStatement();CMI5_SetDurationInStatement(st);CMI5_STATEMENT_QUEUE.push(st);return true;}
...
var uri=new URI(str);if(!uri.is("absolute")){uri=new URI(cmi5.getActivity().id);if(uri.fragment()===""){uri.hash(str);}
else{uri.hash(uri.fragment()+"-"+str);}}
uri.normalize();uri.iri();return uri.toString();}
